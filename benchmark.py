...
class BenchmarkRunner:
    def __init__(
            self, model_config, detail=False, device='cuda', torchscript=False, precision='float32',
            num_warm_iter=10, num_bench_iter=50, **kwargs):
        model_name = osp.splitext(osp.basename(model_config))[0]
        ...
        cfg = _C.clone()
        _update_config_from_file(cfg, model_config)
        ...
        data_config = resolve_data_config(kwargs, model=self.model)
...
class InferenceBenchmarkRunner(BenchmarkRunner):
    ...
    def __init__(self, model_config, device='cuda', torchscript=False, **kwargs):
        super().__init__(model_config=model_config, device=device, torchscript=torchscript, **kwargs)
        self.model.eval()
...
class TrainBenchmarkRunner(BenchmarkRunner):
    ...
    def __init__(self, model_config, device='cuda', torchscript=False, **kwargs):
        ...
        PseudoArgs = namedtuple('PseudoArgs', ['opt', 'lr', 'weight_decay', 'momentum'])
        args = PseudoArgs(opt=kwargs.pop('opt', 'sgd'), lr=kwargs.pop('lr', 1e-4), momentum=0.9, weight_decay=0.)
...
def _try_run(model_config, bench_fn, initial_batch_size, bench_kwargs):
    batch_size = initial_batch_size
    results = dict()
    while batch_size >= 1:
        torch.cuda.empty_cache()
        try:
            bench = bench_fn(model_config=model_config, batch_size=batch_size, **bench_kwargs)
            results = bench.run()
            return results
...
def benchmark(args):
    ...
    model_config = bench_kwargs.pop('model_config')
    ...
    for prefix, bench_fn in zip(prefixes, bench_fns):
        run_results = _try_run(model_config, bench_fn, initial_batch_size=batch_size, bench_kwargs=bench_kwargs)
        if prefix:
...
def main():
    ...
    if args.model_list:
        ...
        for cfg in scandir('configs/', recursive=True, suffix='.yaml'):
            name_mappings[os.path.basename(cfg)] = osp.join('configs', cfg)
        model_cfgs = [n for n in model_names if n in name_mappings]
    ...
    if len(model_cfgs):
        ...
        try:
            for m in model_cfgs:
                args.model = osp.splitext(m)[0]
                args.model_config = name_mappings[m]
                r = benchmark(args)
                results.append(r)
