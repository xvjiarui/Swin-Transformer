...
class BottleneckAttnBlock(nn.Module):
    def __init__(self, mlp_ratio=1., **kwargs):
        super().__init__()
        self.in_block = CrossAttnBlock(mlp_ratio=mlp_ratio, **kwargs)
        self.out_block = CrossAttnBlock(mlp_ratio=mlp_ratio, **kwargs)

    def forward(self, x, y):
        y = self.in_block(y, x)
        x = self.out_block(x, y)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_seq_len, depth, num_heads, num_cluster,
        ...
        self.concat_cluster_token = concat_cluster_token
        ...
        for i in range(depth):
            if i in bottleneck_indices:
                blocks.append(BottleneckAttnBlock(
                    dim=dim, num_heads=num_heads, mlp_ratio=bottleneck_mlp_ratio,
    ...
    def extra_repr(self) -> str:
        return f"dim={self.dim}, \n" \
               ...
               f"concat_cluster_token={self.concat_cluster_token}"
        ...
        if self.concat_cluster_token:
            ...
            for blk_idx, blk in enumerate(self.blocks):
                ...
                if isinstance(blk, BottleneckAttnBlock):
                    ...
                    if self.use_checkpoint:
                        x, cluster_token = checkpoint.checkpoint(blk, x, cluster_token)
                    else:
...
class GroupViT(nn.Module):
    ...
    def __init__(self,
        ...
        for i_layer in range(self.num_layers):
            ...
            layer = BasicLayer(dim=dim,
                               ...
                               bottleneck_indices=bottleneck_indices[i_layer])
        ...
        if 'cluster' in pred_src:
            ...
            self.head_cluster = nn.Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity()
    ...
    @property
    def with_image_pred(self):
        return 'image' in self.pred_src
    ...
    def forward_cluster_head(self, cluster_token):
